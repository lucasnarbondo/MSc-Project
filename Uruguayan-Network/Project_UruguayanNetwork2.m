...
model.obj=[HourlyCost aux_zeros aux_zeros aux_zeros aux_zeros StorageNodeK];
...
for k=1:(TotalSimulation-1)
    ...
    A(MatrixIndex,:)=[aux_zeros_gen aux_A_storage_LIION aux_A_charge_LIION aux_A_discharge_LIION aux_zeros StorageNodeK];
...
for n=1:NumberBuses
    ...
    A(MatrixIndex,:)=[aux_zeros_gen aux_A_storage_LIION aux_zeros aux_zeros aux_zeros StorageNodeK];
...
for i=1:NumberBuses     %Number of equations: TotalSimulation*NumberBuses
    ...
    A(MatrixIndex,:)=[aux_A_production_1 aux_A_production_2 aux_zeros aux_A_charge_LIION aux_A_discharge_LIION aux_A_delta StorageNodeK];
...
for i=1:TotalSimulation
    ...
    A(MatrixIndex,:)=[aux_zeros_gen aux_zeros aux_zeros aux_zeros aux_A_delta StorageNodeK];
...
sense_vector=[sense_vector;'<'];
A(MatrixIndex,:)=[aux_zeros_gen aux_zeros aux_zeros aux_zeros aux_zeros StorageNodeK];
...
hold on
legend('Demand+Exportation','Wind+Solar+Biomass','Hydro','Thermal Production','Storage Level','Interpreter','latex')
grid
...
TotalPower=WindAvg+SolarAvg+BiomassAvg+ThermalProduction+TotalDisChargeRate+SaltoGrandeAvg+RioNegroAvg;
plot(TotalDemand-TotalPower)

fprintf('Li-ION Siting Bus1 = %.2f \n',result.x(30*TotalSimulation+1))
fprintf('Li-ION Siting Bus2 = %.2f \n',result.x(30*TotalSimulation+2))
fprintf('Li-ION Siting Bus3 = %.2f \n',result.x(30*TotalSimulation+3))
fprintf('Li-ION Siting Bus4 = %.2f \n',result.x(30*TotalSimulation+4))
fprintf('Li-ION Siting Bus5 = %.2f \n',result.x(30*TotalSimulation+5))
fprintf('Li-ION Siting Bus6 = %.2f \n',result.x(30*TotalSimulation+6))
fprintf('Li-ION Siting Bus7 = %.2f \n',result.x(30*TotalSimulation+7))
