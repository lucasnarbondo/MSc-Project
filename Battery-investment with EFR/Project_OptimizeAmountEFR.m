...
model.obj=[(-Bat_ANNUITIZED-Maintenance_costs) aux_zeros -SpotVector' SpotVector' Payment_EFR*23*365]; %Maximize revenue adding EFR
model.objcon=-Transmission_fees*Bat_rate; %23 hours a day available for providing reserve
...
for k=1:(TotalSimulation-1)
    ...
    A(MatrixIndex,:)=[0 aux_A_storage aux_A_charge aux_A_discharge 0];
...
aux_A_storage=zeros(1,TotalSimulation);
aux_A_storage(1)=1;
A(MatrixIndex,:)=[0 aux_A_storage aux_zeros aux_zeros -Time_response];
...
for k=1:TotalSimulation
    ...
    b_vector=[b_vector;0];
    sense_vector=[sense_vector;'<'];
    A(MatrixIndex,:)=[-1 aux_A_storage aux_zeros aux_zeros Time_response];
...
LowerBound=[0 aux_zeros aux_zeros aux_zeros 0];

aux_inf=inf*ones(1,TotalSimulation);
aux_rate=Bat_rate*ones(1,TotalSimulation);

UpperBound=[inf aux_inf aux_rate aux_rate Bat_rate];
...
OptimalEFR=result.x(1+3*TotalSimulation+1);
...
fprintf('Optimal EFR = %.2f \n',result.x(1+3*TotalSimulation+1))