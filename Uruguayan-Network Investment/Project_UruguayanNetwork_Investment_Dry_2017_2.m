...
load GenerationAndDemandData_Dry_2017_2
...
% for n=1:NumberBuses
%     StorageNodeK_set_0=ones(1,NumberBuses);
%     A(MatrixIndex,:)=[aux_zeros_gen aux_zeros aux_zeros aux_zeros aux_zeros StorageNodeK_set_0];
%     model.constrnames{MatrixIndex} = sprintf('Aux eq storage 0', n);
%     b_vector=[b_vector;0];
...
for k=1:(TotalSimulation-1)
    ...
    A(MatrixIndex,:)=[aux_zeros_gen aux_A_storage_LIION aux_A_charge_LIION aux_zeros StorageNodeK aux_zeros];
...
for n=1:NumberBuses
    ...
    A(MatrixIndex,:)=[aux_zeros_gen aux_A_storage_LIION aux_zeros aux_zeros StorageNodeK aux_zeros];
...
for i=1:NumberBuses     %Number of equations: TotalSimulation*NumberBuses
    ...
    A(MatrixIndex,:)=[aux_A_production_1 aux_A_production_2 aux_zeros aux_A_charge_LIION aux_A_delta StorageNodeK aux_A_curtailment];
...
for i=1:TotalSimulation
    ...
    A(MatrixIndex,:)=[aux_zeros_gen aux_zeros aux_zeros aux_A_delta StorageNodeK aux_zeros];
...
for k=1:TotalSimulation
    ...
    b_vector=[b_vector;0];
    sense_vector=[sense_vector;'<'];
    A(MatrixIndex,:)=[aux_zeros_gen aux_zeros aux_A_charge_LIION aux_zeros StorageNodeK aux_A_WindCurtailed];
    model.constrnames{MatrixIndex} = sprintf('Storage Level LI-ION <kj: Bus%d Time%d', n, k);
    MatrixIndex=MatrixIndex+1;
end
end
...
for i=1:NumberBuses
    ...
    if ThermalAvg(k) > 0
        ...
        UpperBound=[UpperBound;WindBus(i)*0.3];
    end
    end
end
model.lb=LowerBound;
...
legend('Demand+Exportation','Wind+Solar+Biomass','Hydro','Thermal Production','Storage Level','Wind Previously Curtailed','Location','northeast','Interpreter','latex')
grid
...
baseFileName =  sprintf("All2017Dry-Case2-2.pdf");
...
set(gcf, 'Color', 'w');
folder = 'C:\Users\lucas\Documents\Imperial College\Project\Uruguayan Network Investment\Images';
baseFileName =  sprintf("ChargeandDischarge2017-Case2.pdf");
fullFileName = fullfile(folder, baseFileName);
%export_fig(fullFileName);
...
legend('Thermal pre-battery','Thermal post-battery','Location','southeast','Interpreter','latex')
xticks([12 36 60 84 108 132 156])
xticklabels({'Day 1','Day 2','Day 3','Day 4','Day 5','Day 6','Day 7'})
xtickangle(45)
ylabel('Energy (MWh)','Color','k')
...
set(gcf, 'Color', 'w');
folder = 'C:\Users\lucas\Documents\Imperial College\Project\Uruguayan Network Investment\Images';
baseFileName =  sprintf("ThermalPrePostDry2017-Case2.pdf");
fullFileName = fullfile(folder, baseFileName);
%export_fig(fullFileName);
...
TotalPower=WindAvg+SolarAvg+BiomassAvg+ThermalProduction+SaltoGrandeAvg+RioNegroAvg+TotalWindCurtailed;
